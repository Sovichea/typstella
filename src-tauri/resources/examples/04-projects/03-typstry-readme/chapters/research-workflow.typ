...
A research document is larger than its currently open source file. This chapter is deliberately stored separately while `main.typ` remains the document entry point. In Typstry, opening this file keeps the full-document preview, unless the first-line `// @standalone-preview` directive is present to provide an independent preview for this file.
