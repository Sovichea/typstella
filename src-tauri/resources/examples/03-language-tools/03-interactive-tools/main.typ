...
Typstry offers interactive language tools such as spellcheck and word completion. These are controlled from Settings (`Ctrl+,`) and are independent of script-aware editor navigation.

== Khmer Spellcheck and Completion

Khmer support is bundled by default. Typstry will underline unknown words with a blue informational squiggle. 
...
When you mix languages, Typstry automatically routes each script to the appropriate language provider. The Khmer segmenter only analyzes Khmer characters, and the English dictionary only checks Latin characters.
...
Typstry will download the dictionary and immediately activate it for your documents. You can safely remove dictionaries at any time.
