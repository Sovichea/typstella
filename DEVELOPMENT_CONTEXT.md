...
| **Temporary Preview Tabs** | Opening a full live-sync LSP session for every file peaked in the explorer. | Distinguish `{ temporary: true }` clicks, render a fast-path one-off SVG, and promote to permanent on double-click/edit/outline-interaction. | Avoids excessive memory usage, server restarts, and tab proliferation during workspace browsing. |
| **Temporary Preview DOM Leak** | Wiping the `previewPane` DOM when showing temporary SVG previews. | Add a `hidden` CSS class to the cached live-session iframes and manage a dedicated SVG element separately. | Wiping the pane destroys the live-preview cache. Re-mounting a permanent tab then creates a duplicate iframe, leading to split/overlapping preview panes. |
